...
def train(times=1,timesteps=[9000],taus=[0.005]):
        ...
        for tau in taus:
            for bs in [128,256]:
                for ep in range(times):
                    ...
                    model = SAC("MlpPolicy", train_env, verbose=0, device=device,tau=tau,batch_size=bs,gamma=0.99)
                    ...
                    if interest > 0.5 and sharpe > 0.6:
