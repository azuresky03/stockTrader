...
def train(times=1,timesteps=[9000]):
    ...
    for time_step in timesteps:
        for bs in[50,100]:
            for ep in range(times):
                ...
                model = DDPG("MlpPolicy", train_env, verbose=0, device=device,batch_size=bs,action_noise=action_noise,gamma=0.9)
                model.learn(total_timesteps=time_step,log_interval=4)
                ...
                if interest > 0.7 and sharpe > 0.75:
                    model.save("saved_models/env2-DDPG" + s_int)
